---
title: "CTCF borders versus histone modifications and features - H1"
...
Look whether CTCF borders are distinct from non-CTCF borders. In H1 cells.
...
# Prepare output 
output.dir <- "ts210202_CTCF_borders_versus_histone_modifications_H1"
...
# Write H1 LAD borders with / without CTCF
LAD.borders.h1 <- LAD.borders[["H1"]]


# Write h1 LAD borders with / without CTCF
LAD.borders.h1.ctcf <- LAD.borders.h1[LAD.borders.h1$CTCF == "CTCF"]
LAD.borders.h1.nonctcf <- LAD.borders.h1[LAD.borders.h1$CTCF != "CTCF"]

# Prepare file names and write bed files
LAD.borders.h1.ctcf.file <- file.path(output.dir, "LAD_borders_h1_CTCF.bed")
LAD.borders.h1.nonctcf.file <- file.path(output.dir, "LAD_borders_h1_nonCTCF.bed")

export.bed(LAD.borders.h1.ctcf, LAD.borders.h1.ctcf.file)
export.bed(LAD.borders.h1.nonctcf, LAD.borders.h1.nonctcf.file)

# Prepare list of files
LAD.borders.list <- c(LAD.borders.h1.nonctcf.file, LAD.borders.h1.ctcf.file)



# Also, do this for inwards / outwards borders
LAD.borders.h1.outwards <- LAD.borders.h1[LAD.borders.h1$CTCF_strand == "outwards"]
LAD.borders.h1.inwards <- LAD.borders.h1[LAD.borders.h1$CTCF_strand == "inwards"]
LAD.borders.h1.ambiguous <- LAD.borders.h1[LAD.borders.h1$CTCF_strand == "ambiguous"]

# Prepare file names and write bed files
LAD.borders.h1.outwards.file <- file.path(output.dir, "LAD_borders_h1_outwards.bed")
LAD.borders.h1.inwards.file <- file.path(output.dir, "LAD_borders_h1_inwards.bed")
LAD.borders.h1.ambiguous.file <- file.path(output.dir, "LAD_borders_h1_ambiguous.bed")

export.bed(LAD.borders.h1.outwards, LAD.borders.h1.outwards.file)
export.bed(LAD.borders.h1.inwards, LAD.borders.h1.inwards.file)
export.bed(LAD.borders.h1.ambiguous, LAD.borders.h1.ambiguous.file)

# Prepare list of files
LAD.borders.list.stranded <- c(LAD.borders.h1.nonctcf.file, 
                               LAD.borders.h1.outwards.file,
                               LAD.borders.h1.inwards.file,
                               LAD.borders.h1.ambiguous.file)
...
track_dir <- "/DATA/scratch/usr/t.v.schaik/data/4DNucleome/GRCh38/H1/ENCODE/bigwig/"

tracks <- c("/DATA/scratch/usr/t.v.schaik/proj/3D_nucleus/results/ts180110_4DN_DataProcessing/results/tracks/normalized/bin-10kb/H1_LMNB1-10kb-combined.bw",
            "/DATA/scratch/usr/t.v.schaik/data/4DNucleome/GRCh38/H1/ENCODE/bigwig/H1-ENCODE_ChIPseq_GRCh38_CTCF-human_r1,2_ENCFF473IZV.bigWig",
            "/DATA/scratch/usr/t.v.schaik/data/4DNucleome/GRCh38/H1/ENCODE/bigwig/H1-ENCODE_ChIPseq_GRCh38_RAD21-human_r1,2_ENCFF506AAX.bigWig",
            "/DATA/scratch/usr/t.v.schaik/data/4DNucleome/GRCh38/H1/ENCODE/bigwig/H1-ENCODE_DNAse_GRCh38_r1_ENCFF117WXM.bigWig",
            "/DATA/scratch/usr/t.v.schaik/data/4DNucleome/GRCh38/H1/ENCODE/bigwig/H1-ENCODE_ChIPseq_GRCh38_H3K4me1-human_r1,2_ENCFF591KWL.bigWig",
            "/DATA/scratch/usr/t.v.schaik/data/4DNucleome/GRCh38/H1/ENCODE/bigwig/H1-ENCODE_ChIPseq_GRCh38_H3K4me3-human_r1,2_ENCFF372YWG.bigWig",
            "/DATA/scratch/usr/t.v.schaik/data/4DNucleome/GRCh38/H1/ENCODE/bigwig/H1-ENCODE_ChIPseq_GRCh38_H3K9me3-human_r1,2_ENCFF435YZW.bigWig",
            "/DATA/scratch/usr/t.v.schaik/data/4DNucleome/GRCh38/H1/ENCODE/bigwig/H1-ENCODE_ChIPseq_GRCh38_H3K27ac-human_r1,2_ENCFF423TVA.bigWig",
            "/DATA/scratch/usr/t.v.schaik/data/4DNucleome/GRCh38/H1/ENCODE/bigwig/H1-ENCODE_ChIPseq_GRCh38_H3K27me3-human_r1,2_ENCFF043JFV.bigWig",
            "/DATA/scratch/usr/t.v.schaik/data/4DNucleome/GRCh38/H1/ENCODE/bigwig/H1-ENCODE_ChIPseq_GRCh38_H3K36me3-human_r1,2_ENCFF141YAA.bigWig")

track_names <- c("LMNB1", 
                 str_remove(str_remove(str_remove(basename(tracks[2:length(tracks)]), "H1-ENCODE_"), "ChIPseq_GRCh38_"), "[_|-].*"))
...
tib_gather <- tib %>%
  ...
  add_column(border = 1:nrow(.),
             ctcf = rep(c("nonctcf", "ctcf"), 
                        times = c(length(LAD.borders.h1.nonctcf),
                                  length(LAD.borders.h1.ctcf)))) %>%
...
idx_active <- as_tibble(mcols(c(LAD.borders.h1.nonctcf,
                                LAD.borders.h1.ctcf))) %>%
  pull(ovl_gene)
...
tib_gather <- tib %>%
  ...
  add_column(border = 1:nrow(.),
             ctcf = rep(c("nonctcf", "outwards", "inwards", "ambiguous"), 
                        times = c(length(LAD.borders.h1.nonctcf),
                                  length(LAD.borders.h1.outwards),
                                  length(LAD.borders.h1.inwards),
                                  length(LAD.borders.h1.ambiguous)))) %>%
  gather(key, value, -border, -ctcf) %>%
  separate(key, c("track", "bin"), remove = T) %>%
  mutate(track = factor(track, levels = track_names),
         bin = as.numeric(bin),
...
# Filter borders overlapping with active genes
idx_active <- as_tibble(mcols(c(LAD.borders.h1.nonctcf,
                                LAD.borders.h1.outwards,
                                LAD.borders.h1.inwards,
                                LAD.borders.h1.ambiguous))) %>%
  pull(ovl_gene)
