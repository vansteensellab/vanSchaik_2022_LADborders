...
main <- function(bam, gff, output_dir, basename=NULL, maxgap=0, shift=0,
    ...
    normal_chromosomes <- c(as.character(1:19), "X", "Y")
    ...
    if (! is.null(normal_chromosomes)) {
        ...
        # Mouse data
        seqlevels(gff.ranges, pruning.mode="coarse") <- c(as.character(1:19), "X", "Y")
...
get_bins <- function(gff, genome_size, bin) {
    ...
    # To match UCSC with Ensembl, remove the "chr" start
    g$chr <- gsub("chr", "", g$chr)
